...
async function getBugs(){
  try{
    const res = await fetch(SUPABASE_URL + '/bugs?select=*&order=id.asc', {
      method: 'GET',
      headers: getHeaders()
    });
    if(!res.ok){
      const errText = await res.text();
      window.__bugError = '查询失败: ' + res.status + ' ' + res.statusText + ' - ' + errText;
      return [];
    }
    const data = await res.json();
    window.__bugError = null;
    return data || [];
  }catch(e){
    window.__bugError = '查询异常: ' + e.message;
    return [];
  }
}
...
async function submitBug(e){
  ...
  try{
    const res = await fetch(SUPABASE_URL + '/bugs', {
      method: 'POST',
      headers: {
        ...getHeaders(),
    ...
    var newId = allBugs.length > 0 ? allBugs[allBugs.length - 1].id : 1;
...
async function updateStats(){
  ...
  // 显示错误信息
  var errBox = document.getElementById('debug-error');
  if(!errBox){
    errBox = document.createElement('div');
    errBox.id = 'debug-error';
    errBox.style.cssText = 'position:fixed;bottom:10px;left:10px;right:10px;background:#ff4444;color:white;padding:10px;border-radius:5px;font-size:12px;z-index:9999;display:none;white-space:pre-wrap;word-break:break-all;';
    document.body.appendChild(errBox);
  }
  if(window.__bugError){
    errBox.textContent = '【调试信息】' + window.__bugError;
...
async function changeStatus(id, status){
  try{
    const res = await fetch(SUPABASE_URL + '/bugs?id=eq.' + id, {
      method: 'PATCH',
      headers: getHeaders(),
